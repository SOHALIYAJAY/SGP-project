      ...
      <SectionWrapper>
        ...
        <div className="mb-8 flex items-start justify-between">
          <div className="opacity-0 animate-fade-in-up">
            ...
            <p className="text-muted-foreground">
              {(analysis?.meta.companyName ?? "TechCorp Inc.")} - Analysis generated on{" "}
              {analysis?.meta.generatedAt ? new Date(analysis.meta.generatedAt).toLocaleDateString() : "Feb 3, 2026"}
            </p>
          </div>
          <button
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <MetricCard
            ...
            value={`${analysis?.dashboard.businessHealthScore ?? 87}%`}
            subtitle={(analysis?.dashboard.businessHealthScore ?? 87) >= 75 ? "Above industry average" : "Needs improvement"}
            icon={Activity}
            status="success"
          ...
          <MetricCard
            title="Risk Level"
            value={analysis?.dashboard.riskLevel ?? "Medium"}
            subtitle={analysis?.dashboard.riskLevel === "High" ? "Immediate attention needed" : "3 areas need attention"}
            icon={AlertTriangle}
            status="warning"
            delay={200}
          ...
          <MetricCard
            ...
            value={analysis?.dashboard.investmentReadinessGrade ?? "A-"}
            subtitle={(analysis?.dashboard.investmentReadinessGrade ?? "A-").startsWith("A") ? "Series B ready" : "Strengthen fundamentals"}
            icon={TrendingUp}
            status="success"
          ...
          <MetricCard
            ...
            value={`${analysis?.dashboard.failureProbabilityPercent ?? 18}%`}
            subtitle={(analysis?.dashboard.failureProbabilityPercent ?? 18) >= 55 ? "Elevated risk" : "Within acceptable range"}
            icon={AlertCircle}
            status="warning"
            delay={500}
