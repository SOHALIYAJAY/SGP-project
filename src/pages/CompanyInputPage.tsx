...
export default function CompanyInputPage() {
  ...
  const calculateHealthScore = () => {
    ...
    const revenueGrowth = formData.revenueGrowth[0];
    const profitMargin = formData.profitMargin[0];
    const customerSatisfaction = formData.customerSatisfaction[0];
    const retentionRate = formData.retentionRate[0];
    const automationLevel = formData.automationLevel[0];
    const scalabilityScore = formData.scalabilityScore[0];
    
    const score = Math.round(
      (revenueGrowth * 0.2 + 50) * 0.2 +
      (profitMargin + 50) * 0.15 +
  ...
  const calculateRiskLevel = () => {
    const regulatoryRisk = formData.regulatoryRisk;
    const dependencyRisk = formData.dependencyRisk;
    
    if (regulatoryRisk === "very-high" || regulatoryRisk === "high" || dependencyRisk === "critical" || dependencyRisk === "high") {
      return "High";
    } else if (regulatoryRisk === "moderate" || dependencyRisk === "moderate") {
      return "Medium";
    }
    return "Low";
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const analysis = {
      ...
      industry: getIndustryLabel(formData.industry),
      stage: getStageLabel(formData.companyStage),
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-primary">{formData.marketGrowthRate[0]}%</span>
                </div>
                <Slider
                  value={formData.marketGrowthRate}
                  onValueChange={(value) => handleInputChange("marketGrowthRate", value)}
                  max={100}
                  min={-30}
                  step={1}
                  className="w-full"
          ...
          <div className="form-section">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="regulatoryRisk">Regulatory Risk Level</Label>
                <Select
                  value={formData.regulatoryRisk}
                  onValueChange={(value) => handleInputChange("regulatoryRisk", value)}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                <Label htmlFor="dependencyRisk">Dependency Risk Level</Label>
                <Select
                  value={formData.dependencyRisk}
                  onValueChange={(value) => handleInputChange("dependencyRisk", value)}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select dependency risk" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="minimal">Minimal (Diversified suppliers/partners)</SelectItem>
                    <SelectItem value="low">Low (Some key dependencies)</SelectItem>
                    <SelectItem value="moderate">Moderate (Few critical dependencies)</SelectItem>
                    <SelectItem value="high">High (Single-source dependencies)</SelectItem>
                    <SelectItem value="critical">Critical (High vendor lock-in)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-primary">{formData.scalabilityScore[0]}%</span>
                </div>
                <Slider
                  value={formData.scalabilityScore}
                  onValueChange={(value) => handleInputChange("scalabilityScore", value)}
                  max={100}
                  step={5}
                  className="w-full"
                />
                <p className="text-xs text-muted-foreground">
                  Ability to scale operations without proportional cost increase
                </p>
              </div>
              <div className="space-y-4">
                <div className="flex justify-between">
                  <Label>VC Fit Score</Label>
                  <span className="text-sm font-medium text-primary">{formData.vcFitScore[0]}%</span>
                </div>
                <Slider
                  value={formData.vcFitScore}
                  onValueChange={(value) => handleInputChange("vcFitScore", value)}
                  max={100}
                  step={5}
                  className="w-full"
                />
                <p className="text-xs text-muted-foreground">
                  Alignment with typical VC investment criteria
                </p>
              </div>
              <div className="space-y-2">
                <Label htmlFor="investmentReadiness">Investment Readiness</Label>
                <Select
                  value={formData.investmentReadiness}
                  onValueChange={(value) => handleInputChange("investmentReadiness", value)}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="early-prep">Early Preparation</SelectItem>
                    <SelectItem value="preparing">Actively Preparing</SelectItem>
                    <SelectItem value="ready">Investment Ready</SelectItem>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => handleInputChange("exitStrategy", value)}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="undefined">Not Yet Defined</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                <Label htmlFor="burnMultiple">Burn Multiple</Label>
                <Input
                  id="burnMultiple"
                  placeholder="e.g., 1.5 (Net Burn / Net New ARR)"
                  type="number"
                  step="0.1"
                  value={formData.burnMultiple}
                  onChange={(e) => handleInputChange("burnMultiple", e.target.value)}
                />
                <p className="text-xs text-muted-foreground">
                  Ratio of cash burned to net new revenue added
                </p>
              </div>
              <div className="space-y-2">
